...
class DomainNetDataset(WILDSDataset):
    ...
    def __init__(
        ...
        source_domain: str = ["clipart", "infograph", "painting", "real"],
        target_domain: str = ["quickdraw","sketch"],
        ...
        source_metadata_df = metadata_df.loc[metadata_df["domain"].isin(source_domain)]
        target_metadata_df = metadata_df.loc[metadata_df["domain"].isin(target_domain)]
    ...
    def initialize_split_array(self, metadata_df, source_domain, target_domain):
        # def get_split(row):
        #     if row["domain"] in source_domain:
        #         if row["is_train"]:
        #             return 0
        #         else:
        #             return 3
        ...
        split_map = torch.ones(len(metadata_df), dtype=int) * 3
        split_map[metadata_df['domain'].isin(target_domain).to_numpy()] = 2

        source_indices = np.arange(len(split_map))[split_map == 3]
        split_map[np.random.choice(source_indices, int(len(source_indices)*0.8), replace=False)] = 0

        target_indices = np.arange(len(split_map))[split_map == 2]
        split_map[np.random.choice(target_indices, int(len(target_indices)*0.8), replace=False)] = 1

        self._split_array = split_map
